...
export default class SiderMenu extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    // 2018-07-06：动态加载菜单栏，根据上层组件驱动数据判断是否更新菜单
    if (this.menus !== nextProps.menuData) {
   ...
   *
   * [{path:string},{path:string}] => {path,path2}  20180706 path 属性改为 url 属性
   * @param  menus
   */
  ...
  getNavMenuItems = (menusData) => {
    ...
    return menusData
      ...
      .map(item => {
        ...
        return this.checkPermissionItem(item.perms, ItemDom);
      })
      .filter(item => item);
  };
