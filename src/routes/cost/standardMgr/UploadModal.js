...
import {Button, Col, Input, message, Modal, Progress, Row, Select, Checkbox, Upload} from 'antd';
...
class UploadModal extends React.Component {
  constructor(props) {
    ...
    if(this.state.checkValue !== undefined){
      url = `${this.state.action}/false`
    }else{
  ...
  startValidate = (interval, info) => {
    setTimeout(() => {
      const {validatePercent,progressStatus} = this.state;
      if (info) {
  ...
  handleChange = (info) => {
    ...
    if (response !== undefined) {
      ...
      if (response.code === 0) {
        ...
        //   <span>{response.msg?response.msg:"未知原因"}</span>
        // </div>;
        Modal.error({
          title: "失败",
    ...
    if (info.event && info.event.percent) {
      const percent = info.event.percent.toFixed(2);
  ...
  onCheckChange = (e) =>{
    ...
    url = `${this.state.action  }/${  e.target.checked}`
  }
  ...
  render() {
    ...
    const uploadProps = {
      action: `http://localhost:8080/api${url}`,
    ...
    return (
      ...
      >
        ...
        <Row gutter={24} style={{marginTop:"10px"}} type="flex" align="middle">
          {/* <Col span={2}/> */}
          <Col span={4} >选择文件：</Col>
          <Col span={8}><Input style={{width:"100%"}} value={uploadFile && uploadFile.name} disabled /></Col>
        ...
        {checkValue !== undefined && <Row gutter={24} style={{marginTop:"10px"}} type="flex" align="middle">
          <Col span={24}><Checkbox onChange={this.onCheckChange}>{this.props.checkValue}</Checkbox></Col>
        </Row>}
