...
const CreateForm = Form.create({
  ...
  const { modalVisible, form, handleSave, handleModalVisible, handleRoleChecked, roleList, userOnForm } = props;
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="角色">
        {
          <CheckboxGroup options={roleCheckboxOptions} value={roleIds} onChange={checkedHandle} />
        }
      </FormItem>
    </Modal>
  );
...
export default class SysUser extends PureComponent{
  ...
  render(){
    ...
    const { sysUser: { userList = [], roleList = [], pagination }, loading } = this.props;
